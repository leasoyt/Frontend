...
import { getRestaurantById } from '@/scripts/user/get-restaurant';
...
const RestaurantPage = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      getRestaurantById(pathArray[pathArray.length - 1]).then((data) => {
        if (data) {
          setRestaurant(data);
        }
      }).catch((error) =>
  ...
  const handleCreateReservation = async () => {
    ...
    const reservationData = {
      ...
      restaurant_id: restaurant?.id, // Asegúrate de que sea un string
                  ...
                  restaurant.menu && restaurant.menu.categories ?
                    (
                      restaurant.menu.categories.flatMap((category: IMenu_Category) =>
                        category.dishes?.map((dish: IDish) => (
